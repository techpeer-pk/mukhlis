...
export default function InstallPrompt() {
  ...
  }, [prompt, ios, installed, dismissed])
  ...
  const install = () => { triggerInstall(); setShow(false) }
        ...
        {prompt && (
          <>
            ...
            <div className="flex gap-2">
              <button onClick={install} className="flex-1 flex items-center justify-center gap-2 bg-slate-800 dark:bg-slate-700 text-white font-semibold py-3 rounded-xl text-sm transition-all">
                <DownloadIcon className="w-4 h-4" /> {t('install')}
              </button>
              <button onClick={dismiss} className="px-4 border border-gray-200 dark:border-gray-600 text-slate-500 dark:text-slate-400 rounded-xl text-sm hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                {t('later')}
              </button>
            </div>
          </>
            ...
            <ol className="space-y-2 mb-4">
              {[t('ios_step1'), t('ios_step2'), t('ios_step3')].map((step, i) => (
                <li key={i} className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
                  ...
                  {i === 0 && <><span>{step}</span><ShareIcon className="w-4 h-4 mx-1 text-blue-500" /></>}
                  {i !== 0 && step}
                </li>
              ))}
            </ol>
            <button onClick={dismiss} className="w-full border border-gray-200 dark:border-gray-600 text-slate-500 dark:text-slate-400 py-3 rounded-xl text-sm hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
              {t('ok')}
            </button>
          </>
        )}
