    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex flex-col items-center px-4 py-8 transition-colors">
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-xl font-bold text-slate-800 dark:text-slate-100">{t('my_reports_title')}</h1>
          <p className="text-slate-400 dark:text-slate-500 text-xs mt-1">{t('my_reports_sub')}</p>
        </div>

        {!loading && docs.length > 0 && (
          <div className="grid grid-cols-3 gap-2 mb-6">
            {[
              { labelKey: 'status_active',  count: counts.available, color: 'text-emerald-600 dark:text-emerald-400' },
              { labelKey: 'status_claimed', count: counts.claimed,   color: 'text-blue-600 dark:text-blue-400' },
              { labelKey: 'status_expired', count: counts.expired,   color: 'text-gray-400 dark:text-gray-500' },
            ].map(s => (
              <div key={s.labelKey} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-3 text-center shadow-sm">
                <div className={`text-2xl font-bold ${s.color}`}>{s.count}</div>
                <div className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{t(s.labelKey)}</div>
              </div>
            ))}
          </div>
        )}
        ...
        {loading && (
          <div className="text-center py-16 text-slate-400 dark:text-slate-600 text-sm">{t('loading')}</div>
        )}
        ...
        {!loading && docs.length === 0 && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm p-8 text-center">
            ...
            <p className="text-slate-600 dark:text-slate-300 font-medium text-sm">{t('no_reports')}</p>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{t('no_reports_sub')}</p>
              ...
              <div key={doc.id} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm p-4">
                ...
                {doc.status === 'available' && (
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-3 pt-2 border-t border-gray-100 dark:border-gray-700">
                    {t('expires')}: {new Date(doc.expiresAt).toLocaleDateString('en-PK')}
                  </p>
                )}
              </div>
            ))}
